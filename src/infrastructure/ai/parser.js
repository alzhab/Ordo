...
- "open_plan" — открыть/показать план на конкретный день ("покажи план на сегодня", "покажи план на пятницу", "открой план на 15 апреля", "свободен ли я в четверг", "что у меня на след неделе в четверг")
- "open_review" — открыть разбор зависших задач ("давай сделаем разбор", "открой разбор", "покажи зависшие", "что зависло", "разбор задач")
- "unplan_tasks" — убрать задачи из плана на конкретный день ("убери все задачи из плана", "освободи 13 апреля", "очисти план на сегодня", "убери всё с пятницы")
...
Поле "reminder_at" — время события или напоминания. Устанавливай в двух случаях:
1. Задача или событие имеет конкретное время: "встреча в 15:00", "посиделка в 20:00", "звонок в 10 утра", "обед с Сашей в 13:30" → reminder_at = это время на дату plannedFor в формате "YYYY-MM-DD HH:MM"
2. Пользователь явно просит напоминание: "напомни", "напомни мне", "поставь напоминание", "напомни за N минут/часов до"
Дополнительные правила:
- "через N минут" / "через N часов" — верни reminder_at ДОСЛОВНО: "через N минут" или "через N часов" (сервер посчитает точное время)
- "за N минут/часов до события" — вычисли время события минус N → верни "YYYY-MM-DD HH:MM"
...
- "Посиделка с бекой сегодня в 20:00" → plannedFor: сегодня, reminder_at: "<сегодня> 20:00"
- "Встреча в пятницу в 15:00" → plannedFor: пятница, reminder_at: "<пятница> 15:00"
- "Звонок маме завтра в 10 утра" → plannedFor: завтра, reminder_at: "<завтра> 10:00"
...
- "Купить молоко завтра" → reminder_at: null (нет конкретного времени)
...
Если intent = "query_tasks", верни:
{
  ...
  "date": "today" | "week" | "no_date" | ISO date string | null
}
Значения поля date:
- "today" — задачи на сегодня
- "week" — задачи на текущую неделю
- "no_date" — задачи без запланированной даты ("задачи без даты", "задачи без планировки")
- ISO date string ("2026-04-15") — задачи на конкретную дату ("задачи на 15 апреля", "задачи на следующую пятницу", "задачи на след неделе в четверг")
Примеры query_tasks:
- "покажи задачи по дому" → category: "Дом"
- "что в работе?" → status: "in_progress"
- "задачи на сегодня" → date: "today"
- "задачи на след неделе в четверг" → date: "<ближайший четверг следующей недели ISO>"
- "задачи без даты" / "покажи задачи без планировки" → date: "no_date"
...
Если intent = "open_plan", верни:
{
  "intent": "open_plan",
  "date": ISO date string | null
}
date — конкретная дата (вычисли из относительной: "в пятницу", "на следующей неделе в четверг", "15 апреля").
null — если пользователь имеет в виду сегодня или не уточнил дату.
Примеры open_plan:
- "покажи план на сегодня" → date: null
- "что у меня на след неделе в четверг" → date: "<ближайший четверг следующей недели ISO>"
- "свободен ли я в пятницу" → date: "<ближайшая пятница ISO>"
- "план на 25 апреля" → date: "2026-04-25"
- "открой план" → date: null

Если intent = "open_review", верни:
{
  "intent": "open_review"
}
Примеры open_review:
- "давай сделаем разбор" → intent: "open_review"
- "покажи зависшие" → intent: "open_review"
- "что у меня зависло" → intent: "open_review"
- "разбор задач" → intent: "open_review"
- "открой разбор" → intent: "open_review"

Если intent = "unplan_tasks", верни:
{
  "intent": "unplan_tasks",
  "date": ISO date string | null
}
date — конкретная дата. null — если не уточнена (сегодня).
Примеры unplan_tasks:
- "убери все задачи из плана" → date: null
- "освободи 13 апреля без задач" → date: "2026-04-13"
- "очисти план на сегодня" → date: null
- "убери всё с пятницы" → date: "<ближайшая пятница ISO>"
